...
import torch
# from torch import cat as pt_cat, zeros as pt_zeros, ones as pt_ones, from_numpy, Tensor
...
def clotho_collate_fn(batch: MutableSequence[ndarray]):
    ...
    # Pad the audios to have the same length
    audio_lengths = [audio.size(1) for audio in audios]
    max_audio_length = max(audio_lengths)
    padded_audios = [torch.nn.functional.pad(audio, (0, max_audio_length - audio.size(1))) for audio in audios]

    # Stack the audios
    padded_audios = torch.stack(padded_audios)

    return padded_audios, captions
